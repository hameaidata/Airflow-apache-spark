...
# ============================================================================
# VERSIÓN CORREGIDA: Incluye estructura completa de Spark + validación mejorada
# ============================================================================
...
echo -e "${CYAN}=== Preparando stack Airflow + Spark (modo: ${MODO}) - VERSIÓN MEJORADA ===${NC}"
...
# --- 5. Crear estructura de carpetas (MEJORADO) ----------------------------
# Se agregan subdirectorios de spark/jobs que estaban faltando
...
ok "estructura de carpetas creada (incluye spark/jobs/etl, analytics, transformations)"
...
if [ -f .env ]; then
    ...
    # MEJORA: Incluir SPARK_AUTH_SECRET en el template
    cat > .env <<EOF
# ============================================================================
...
EOF
    chmod 600 .env
    ok ".env generado con secretos aleatorios (AIRFLOW_UID=${CURRENT_UID}, incluye SPARK_AUTH_SECRET, permisos 600)"
fi
...
# --- 9. Resumen (MEJORADO CON INFORMACIÓN DE CAMBIOS) ----------------------
...
MEJORAS en esta version:
  ✓ Estructura completa de spark/jobs/ (etl, analytics, transformations)
  ✓ Directorio spark/libs/ para librerías externas
  ✓ SPARK_AUTH_SECRET generado en .env para autenticación futura
  ✓ Validación mejorada de estructura de carpetas
